...
class TestArray2D:
    ...
    # ✅ Test for ValueError if `starting_sequence` is not a sequence of sequences
    def test_init_not_sequence(self) -> None:
        ...
        with pytest.raises(ValueError):
            _ = Array2D("invalid_string", data_type=str)  # Not a sequence of sequences

        with pytest.raises(ValueError):
            _ = Array2D({1: [1, 2, 3]}, data_type=dict)  # Dictionary is not a valid sequence
